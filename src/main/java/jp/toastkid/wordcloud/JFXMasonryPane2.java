...
/**
 * This class is used for avoiding ununderstandable error.
 *
...
//  private GridPane root = new GridPane();
    ...
    protected void layoutChildren() {
        ...
        if(!valid){
            ...
            final List<Region> childs = new ArrayList<>();
            for(int i = 0 ; i < getChildrenUnmodifiable().size(); i++) {
                if(getChildrenUnmodifiable().get(i) instanceof Region) {
                    childs.add((Region) getChildrenUnmodifiable().get(i));
                }
            }
            ...
            for (int i = 0; i < getManagedChildren().size() && i < boxes.size(); i++) {
                final Region block = (Region) getManagedChildren().get(i);
            ...
            trans.stop();
            final ParallelTransition newTransition = new ParallelTransition();
...
//              root.resizeRelocate(0, 0, this.getWidth(), this.getHeight());
     ...
     */
    private final IntegerProperty limitRow = new SimpleIntegerProperty(-1);
    ...
    protected interface Layout {
         ...
         * @return
         */
        default BoundingBox getFreeArea(final int[][] matrix, final int x, final int y, final Region block, final double cellWidth, final double cellHeight, final int limitRow, final int limitCol, final double gutterX, final double gutterY){
            final double blockHeight = getBLockHeight(block);
            final double blockWidth = getBLockWidth(block);

            int rowsNeeded = (int)Math.ceil(blockHeight/(cellHeight + gutterY));
            if(cellHeight*rowsNeeded + (rowsNeeded - 1)*2*gutterY < blockHeight) {
                rowsNeeded++;
            }
            final int maxRow = Math.min(x + rowsNeeded, limitRow);

            int colsNeeded = (int)Math.ceil(blockWidth/(cellWidth + gutterX));
            if(cellWidth*colsNeeded + (colsNeeded - 1)*2*gutterX < blockWidth) {
                colsNeeded++;
            }
            final int maxCol = Math.min(y + colsNeeded, limitCol);
        ...
        default double getBLockWidth(final Region region){
        ...
        default double getBLockHeight(final Region region){
        ...
        default boolean validWidth(final BoundingBox box , final Region region, final double cellW, final double gutterX, final double gutterY){
            boolean valid = false;
        ...
        default boolean validHeight(final BoundingBox box , final Region region, final double cellH, final double gutterX, final double gutterY){
            boolean valid = false;
        ...
        default int[][] fillMatrix(final int[][] matrix, final int id, final double row, final double col, final double width, final double height){
            //          int maxCol = (int) col;
            //          int maxRow = (int) row;
