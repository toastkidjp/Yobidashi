...
public final class JapaneseHoliday{
     ...
     * @return
     */
    public static Set<Integer> findHolidays(final int year, final int month) {
     ...
     */
    public static Set<Integer> listHolidays(final int year, final int month){
        if (month < 1 || 12 < month){
        ...
        // 現在、国民の休日の発生は９月しかない
        if (month == SEPTEMBER){
            getNationalHoliday(year).stream().map(ld -> ld.getDayOfMonth()).forEach(set::add);
        }
        return set;
    }
    ...
    public static Set<LocalDate> listHoliDayDates(final int year, final int month){
        ...
        final Set<LocalDate> set = new LinkedHashSet<>();
        for (int i = 0; i < constructors.length; i++){
            try{
                final HolidayBundle b = (HolidayBundle)constructors[i].newInstance(null,year);
                set.add(b.getDate());
        ...
        // 現在、国民の休日の発生は９月しかない
        if (month == SEPTEMBER){
            set.addAll(getNationalHoliday(year));
        }
        return set;
    }
    ...
    public static String queryHoliday(final LocalDate ld) {
        ...
        if (getNationalHoliday(targetYear).stream().map(holiday -> YMD_FORMAT.format(holiday))
                        .anyMatch(targetDateStr::equals)) {
            return "国民の休日";
        }
        return null;
    }
     ...
     */
    public static List<LocalDate> getNationalHoliday(final int year){
        ...
        if ((aday - kday) == 2) {
            return Arrays.asList(LocalDate.of(year, SEPTEMBER, kday + 1));
        }

        if (chgday > 0 && ((aday - chgday) == 2)) {
            return Arrays.asList(LocalDate.of(year, SEPTEMBER, chgday + 1));
        }
